...
address_index: [128 * 1024]i32

output_kinds: [dynamic]Line_Data
output_lines: [dynamic]strings.Builder

_parse :: proc(output_file: ^os.File, input: string) {
	get_line :: proc(iter: ^string) -> string {
		n := strings.index_byte(iter^, '\n')
		if n == -1 {
			return ""
		}
		line := iter[:n]
	...
	for iter != "" {
		...
		// type
		if line[0] != ' ' {
			n := strings.index_byte(line, ':')
			type := line[:n]
			line = line[n:]

			switch type {
			case "CODE", "BO", "B1", "B2":
				bin_loc = _parse_code(line, type)
			}
...
_skip_space :: proc(line: string) -> (string, int) {
	for c, i in line {
		if c != ' ' {
			return line[i:], i
		}
...
_parse_code :: proc(line: string, bank: string) -> uint {
	line := line
	...
	for c, i in line {
		if c == ':' {
			...
			ok: bool
			address, ok = strconv.parse_uint(line[last_colon + 1:i], 16)
			assert(ok)
			line = line[i:]
			break
		}
	...
	line, _ = _skip_space(line)

	nibbles := strings.index_byte(line, ' ')
	if nibbles == -1 {
		return ~uint(0)
	}
	fmt.assertf(nibbles % 2 == 0, "odd nibbles @ %u", address)
	bytes := line[:nibbles]
	line = line[nibbles:]

	switch bytes {
	case LJMP_TO_BANK_JUMP_0:
	case LJMP_TO_BANK_JUMP_1:
	case LJMP_TO_BANK_JUMP_2:
	case LJMP_TO_BANK_JUMP_3:
	}

	leading_space: int
	line, leading_space = _skip_space(line)

	code := line
	comment: string
	if n := strings.index_byte(line, ';'); n != -1 {
		comment = line[n:]
		code = code[:n]
	}
	...
	dptr: uint
	mov_dptr: {
		// only care about MOV dptr, ...
		instr_len := strings.index_byte(line, ' ')
		assert(instr_len != -1)

		instruction := line[:instr_len]
		if instruction != "MOV" {
			break mov_dptr
		}
		code_iter, _ := _skip_space(code)

		dest_len := strings.index_byte(code_iter, ',')
		assert(dest_len != -1)

		if code_iter[:dest_len] != "DPTR" {
			break mov_dptr
		}

		code_iter = code_iter[:dest_len + 1]
		if code_iter[:3] != "#0x" {
			break mov_dptr
		}
		code_iter = code_iter[3:]

		ok: bool
		if dptr, ok = strconv.parse_uint(code_iter[:4], 16); !ok {
			dptr = 0
		}
	}

	sb: strings.Builder
	fmt.sbprintf(&sb, "%s:%04x  %s%s%s", bank, address, bytes, leading_space, code, comment)
	...
	address_index[address] = i32(len(output_kinds))

	append(&output_kinds, line_data)
	append(&output_lines, sb)

	return address + uint(nibbles / 2)
}
